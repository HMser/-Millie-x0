...
const mlbase = require('./Base');
const mlmessage = require('./Message');
const mlreplyMessage = require('./ReplyMessage');

class mlvideo extends mlbase {
    ...
    _patch(data) {
        ...
        if (data.message.videoMessage.hasOwnProperty('contextInfo') && data.message.contextInfo.quotedMessage) { 
            this.reply_message = new mlreplyMessage(this.client, data.message.videoMessage.contextInfo); }
        else {
    ...
    async reply(text) {
        ...
        return new mlmessage(this.client, message)
    }
