...
var PROP = { aredits: '918113921898,0' }
var unlink = `019112af334adceaefd1467dcbd93e58` 
var PROP2 = { kl11: '918113921898,0' }
var string = base + `neera-j`
...
async function whatsAsena () {
    ...
    const sourav = await axios('https://gist.githubusercontent.com/neera-j/2b76750a14a1319037ab31515428dd70/raw/17e00a29de97d7a5b979ccff9a9594cb1dc644af/Version.json')
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '-- BUG REPORT [MILLIE] --' +
                                '\n*Millie an error has occurred!*'+
                                '\n_This error log may contain your number or the number of a counterparty. Please pay attention to this!_' +
                                '\n_This message must have gone to your number (saved messages)._\n\n' +
                                'Error Occurred: ' + error + '\n\n'
                                    , MessageType.text);
                            } else {
                                await conn.sendMessage(conn.user.jid, '*~_________~ *Millie ERROR FOUND!* ~______~*' +
                                    '\n\n*🧞‍♂️ ' + error + '*\n\n' 
                                    , MessageType.text);
                            }
                        }
